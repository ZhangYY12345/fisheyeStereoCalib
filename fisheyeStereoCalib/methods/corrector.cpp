	...
	case Forward:
		...
		for (int j = top; j < top + 2 * radius; j++)
		{
			for (int i = left; i < left + 2 * radius; i++)
			{
				...
				//convert to polar axes
				theta = cv::fastAtan2(y_cart, x_cart)*PI / 180;
				...
				latitude = acos(y);
				longitude = cv::fastAtan2(z, -x)*PI / 180;
	...
	case Reverse:
		...
		for (int j = 0; j < imgSize.height; j++)
		{
			...
			for (int i = 0; i < imgSize.width; i++)
			{
				...
				Theta_sphere = acos(z);
				Phi_sphere = cv::fastAtan2(y, x);//return value in Angle
