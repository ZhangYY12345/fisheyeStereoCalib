...
{
	cv::namedWindow(check_win_name, cv::WINDOW_NORMAL);
...
void findCircleParameter::findPoints(Point2i center, int radius, std::vector<cv::Point> &points, camMode projMode)
{
	...
	while (it != points.end())
	{
		...
		double theta = cv::fastAtan2(y_cart, x_cart)*PI / 180;
		...
		case ORTHOGONAL:
			...
			break;
		default:
			std::cout << "The camera mode hasn't been choose!" << std::endl;
		}
		...
		std::cout << "[x, y, z] = " << *itSphere << std::endl
			<< "norm = " << sqrt(temp) << std::endl;
	...
	while (l++ <= count)
	{
		...
		double Phi_sphere = cv::fastAtan2(y, x)*PI / 180;//return value in Angle
		////////////////////////////////////////////////////////////////////////////////
