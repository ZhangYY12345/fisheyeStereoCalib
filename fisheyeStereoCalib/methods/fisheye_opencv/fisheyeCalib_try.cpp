...
cv::Mat my_cv::internal::NormalizePixels(const cv::Mat& imagePoints, const IntrinsicParams& param, DISTORT_Mode_Fisheye distortMode)
{
	...
	case RADIUS_D_FISHEYE_CALIB:
		//fisheye_r_d::
		my_cv::fisheye_r_d::undistortPoints(distorted, undistorted, cv::Matx33d::eye(), param.k, cv::noArray(), cv::noArray(), cur_fisheye_mode);
		break;
	case RADIUS_RD_FISHEYE_CALIB:
		//fisheye_r_rd::
...
void my_cv::internal::InitExtrinsics(const cv::Mat& _imagePoints, const cv::Mat& _objectPoints, const IntrinsicParams& param, cv::Mat& omckk, cv::Mat& Tckk, DISTORT_Mode_Fisheye distortMode)
{
	...
	const double thresh_cond, cv::InputOutputArray omc, cv::InputOutputArray Tc, DISTORT_Mode_Fisheye distortMode)
{
	...
	for (int image_idx = 0; image_idx < (int)imagePoints.total(); ++image_idx)
	{
		...
		InitExtrinsics(imT ? image.t() : image, obT ? object.t() : object, param, omckk, Tckk, distortMode);

		ComputeExtrinsicRefine(!imT ? image.t() : image, !obT ? object.t() : object, omckk, Tckk, JJ_kk, maxIter, param, thresh_cond, distortMode);
		if (check_cond)
		{
        ...
        cv::InputArray _tvec, cv::InputArray _K, cv::InputArray _D, double alpha, cv::OutputArray jacobian, camMode mode)
{
    ...
    for(size_t i = 0; i < n; ++i)
    {
        ...
        if (isJacobianNeeded)
        {
			...
			{
			case STEREOGRAPHIC:
				dcdistdom = inv_r_ * ((1.0 / pow(cos(theta_d / 2.0), 2)) * dtheta_ddom - cdist * dr_dom);	//r = 2f * tan(theta / 2)
				dcdistdT = inv_r_ * ((1.0 / pow(cos(theta_d / 2.0), 2)) * dtheta_ddT - cdist * dr_dT);
				dcdistdk = inv_r_ * (1.0 / pow(cos(theta_d / 2.0), 2)) * dtheta_ddk;
				break;
			case EQUIDISTANCE:
				dcdistdom = inv_r_ * (dtheta_ddom - cdist * dr_dom);	// r = f * theta
				dcdistdT = inv_r_ * (dtheta_ddT - cdist * dr_dT);
				dcdistdk = inv_r_ *  dtheta_ddk;
				break;
			case EQUISOLID:
				dcdistdom = inv_r_ * (cos(theta_d / 2.0) * dtheta_ddom - cdist * dr_dom);	// r = 2f * sin(theta / 2)
				dcdistdT = inv_r_ * (cos(theta_d / 2.0) * dtheta_ddT - cdist * dr_dT);
				dcdistdk = inv_r_ * cos(theta_d / 2.0) * dtheta_ddk;
				break;
			case ORTHOGONAL:
				dcdistdom = inv_r_ * (cos(theta_d) * dtheta_ddom - cdist * dr_dom);	// r = f * sin(theta)
				dcdistdT = inv_r_ * (cos(theta_d) * dtheta_ddT - cdist * dr_dT);
				dcdistdk = inv_r_ * cos(theta_d) * dtheta_ddk;
				break;
			default:
				dcdistdom = inv_r_ * ((1.0 / pow(cos(theta_d), 2)) * dtheta_ddom - cdist * dr_dom);	//r = f * tan(theta)
				dcdistdT = inv_r_ * ((1.0 / pow(cos(theta_d), 2)) * dtheta_ddT - cdist * dr_dT);
				dcdistdk = inv_r_ * (1.0 / pow(cos(theta_d), 2)) * dtheta_ddk;
			}
	...
	cv::InputArray K, cv::InputArray D, cv::InputArray R, cv::InputArray P, camMode mode)
{
    ...
    for(size_t i = 0; i < n; i++ )
    {
		...
		{
		case STEREOGRAPHIC:
			theta_d = 2 * atan(r_d / 2);
			break;
		case EQUIDISTANCE:
			theta_d = r_d;
			break;
		case EQUISOLID:
			theta_d = 2 * asin(r_d / 2);
			break;
		case ORTHOGONAL:
			theta_d = asin(r_d);
			break;
        ...
        if (theta_d > 1e-8)
        {
			...
			{
			case STEREOGRAPHIC:
				scale = 2 * std::tan(theta / 2) / r_d;		//r = 2f * tan(theta / 2)
				break;
			case EQUIDISTANCE:
				scale = theta / r_d;						// r = f * theta
				break;
			case EQUISOLID:
				scale = 2 * std::sin(theta / 2) / r_d;		// r = 2f * sin(theta / 2)
				break;
			case ORTHOGONAL:
				scale = std::sin(theta) / r_d;				// r = f * sin(theta)
				break;
			default:
				scale = std::tan(theta) / r_d;				//r = f * tan(theta)
			}
...
void my_cv::fisheye_r_rd::projectPoints(cv::InputArray objectPoints, cv::OutputArray imagePoints, const cv::Affine3d& affine,
	cv::InputArray K, cv::InputArray D, double alpha, cv::OutputArray jacobian, camMode mode)
{
	projectPoints(objectPoints, imagePoints, affine.rvec(), affine.translation(), K, D, alpha, jacobian, mode);
}

void my_cv::fisheye_r_rd::projectPoints(cv::InputArray objectPoints, cv::OutputArray imagePoints, cv::InputArray _rvec,
	cv::InputArray _tvec, cv::InputArray _K, cv::InputArray _D, double alpha, cv::OutputArray jacobian, camMode mode)
{
	// will support only 3-channel data now for points
	CV_Assert(objectPoints.type() == CV_32FC3 || objectPoints.type() == CV_64FC3);
	...
	for (size_t i = 0; i < n; ++i)
	{
		...
		if (r > 1e-8)
		{
			...
			for (int j = 0; j < 10; j++)
			{
				double r_d2 = r_d * r_d, r_d4 = r_d2 * r_d2, r_d6 = r_d4 * r_d2, r_d8 = r_d6 * r_d2;
				double k0_r_d2 = k[0] * r_d2, k1_r_d4 = k[1] * r_d4, k2_r_d6 = k[2] * r_d6, k3_r_d8 = k[3] * r_d8;
				/* new_r = r - r_fix, r_fix = f0(r) / f0'(r) *///牛顿迭代法求解多项式
				double r_d_fix = (r_d * (1 + k0_r_d2 + k1_r_d4 + k2_r_d6 + k3_r_d8) - r) /
					(1 + 3 * k0_r_d2 + 5 * k1_r_d4 + 7 * k2_r_d6 + 9 * k3_r_d8);
				r_d = r_d - r_d_fix;
				if (fabs(r_d_fix) < EPS)
					break;
			}
		...
		if (isJacobianNeeded)
		{
			...
			double r_d2 = r_d * r_d, r_d3 = r_d2 * r_d, r_d4 = r_d2 * r_d2, r_d5 = r_d3 * r_d2;
			double r_d6 = r_d4 * r_d2, r_d7 = r_d5 * r_d2, r_d8 = r_d6 * r_d2, r_d9 = r_d7 * r_d2;
			//double r = r_d + k[0]*r_d3 + k[1]*r_d5 + k[2]*r_d7 + k[3]*r_d9;
			double drdr_d = 1 + 3 * k[0] * r_d2 + 5 * k[1] * r_d4 + 7 * k[2] * r_d6 + 9 * k[3] * r_d8;
			cv::Vec3d dr_ddom = drdr_d * drdom;
			cv::Vec3d dr_ddT = drdr_d * drdT;
			cv::Vec4d dr_ddk = cv::Vec4d(r_d3, r_d5, r_d7, r_d9);
	...
	cv::OutputArray R, cv::OutputArray T, int flags, cv::TermCriteria criteria)
{
	...
	for (int iter = 0; ; ++iter)
	{
		...
		for (int image_idx = 0; image_idx < n_images; ++image_idx)
		{
			...
			cv::Mat rvec = cv::Mat(rvecs1[image_idx]);
			cv::Mat tvec = cv::Mat(tvecs1[image_idx]);
			my_cv::internal::projectPoints(object, projected, rvec, tvec, intrinsicLeft, jacobians, RADIUS_RD_FISHEYE_CALIB);
			cv::Mat(cv::Mat((imageLeft - projected).t()).reshape(1, 1).t()).copyTo(ekk.rowRange(0, 2 * n_points));
			jacobians.colRange(8, 11).copyTo(Jkk.colRange(24 + image_idx * 6, 27 + image_idx * 6).rowRange(0, 2 * n_points));
			jacobians.colRange(11, 14).copyTo(Jkk.colRange(27 + image_idx * 6, 30 + image_idx * 6).rowRange(0, 2 * n_points));
			jacobians.colRange(0, 2).copyTo(Jkk.colRange(0, 2).rowRange(0, 2 * n_points));
			...
			my_cv::internal::projectPoints(object, projected, omr, Tr, intrinsicRight, jacobians, RADIUS_RD_FISHEYE_CALIB);
			cv::Mat(cv::Mat((imageRight - projected).t()).reshape(1, 1).t()).copyTo(ekk.rowRange(2 * n_points, 4 * n_points));
			cv::Mat dxrdom = jacobians.colRange(8, 11) * domrdom + jacobians.colRange(11, 14) * dTrdom;
			cv::Mat dxrdT = jacobians.colRange(8, 11) * domrdT + jacobians.colRange(11, 14)* dTrdT;
			cv::Mat dxrdomckk = jacobians.colRange(8, 11) * domrdomckk + jacobians.colRange(11, 14) * dTrdomckk;
