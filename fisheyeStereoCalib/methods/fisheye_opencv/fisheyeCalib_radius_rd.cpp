	...
	cv::InputArray _tvec, cv::InputArray _K, cv::InputArray _D, double alpha, cv::OutputArray jacobian, camMode mode)
{
	...
	for (size_t i = 0; i < n; ++i)
	{
		...
		double r_d3 = r_d2 * r_d, r_d5 = r_d2 * r_d3, r_d7 = r_d5 * r_d2, r_d9 = r_d7 * r_d2, r_d11 = r_d9 * r_d2, r_d13 = r_d11 * r_d2;
		double r = r_d + k[0] * r_d3 + k[1] * r_d5 + k[2] * r_d7 + k[3] * r_d9 + k[4] * r_d11 + k[5] * r_d13;
		...
		if (isJacobianNeeded)
		{
			...
			//
			double r_d4 = r_d2 * r_d2, r_d6 = r_d4 * r_d2, r_d8 = r_d6 * r_d2, r_d10 = r_d8 * r_d2, r_d12 = r_d10 * r_d2;
			double drdr_d = 1 + 3 * k[0] * r_d2 + 5 * k[1] * r_d4 + 7 * k[2] * r_d6 + 9 * k[3] * r_d8 + 11 * k[4] * r_d10 +13* k[5] * r_d12;
			cv::Vec2d drdc = drdr_d * dr_ddc;
			cv::Vec2d drdf = drdr_d * dr_ddf;
			double drdalpha = drdr_d * dr_ddalpha;
			cv::Vec6d drdk = cv::Vec6d(r_d3, r_d5, r_d7, r_d9, r_d11, r_d13);
			...
			cv::Vec6d dthetadk = dthetadr * drdk;
	...
	cv::InputArray K, cv::InputArray D, cv::InputArray R, cv::InputArray P, camMode mode)
{
	...
	for (size_t i = 0; i < n; i++)
	{
		...
		double r_d = sqrt(r_d2);
		double r_d3 = r_d2 * r_d, r_d5 = r_d3 * r_d2, r_d7 = r_d5 * r_d2, r_d9 = r_d7 * r_d2, r_d11 = r_d9 * r_d2, r_d13 = r_d11 * r_d2;
		double r = r_d + k[0] * r_d3 + k[1] * r_d5 + k[2] * r_d7 + k[3] * r_d9 +k[4] * r_d11 +k[5] * r_d13;
	...
	cv::OutputArray R, cv::OutputArray T, int flags, cv::TermCriteria criteria)
{
	...
	cv::Mat J = cv::Mat::zeros(6 * n_points * n_images, 22 + 6 * (n_images + 1), CV_64FC1),
	...
	for (int iter = 0; ; ++iter)
	{
		...
		J.create(6 * n_points * n_images, 22 + 6 * (n_images + 1), CV_64FC1);
		e.create(6 * n_points * n_images, 1, CV_64FC1);
		Jkk.create(6 * n_points, 22 + 6 * (n_images + 1), CV_64FC1);
		...
		for (int image_idx = 0; image_idx < n_images; ++image_idx)
		{
			...
			jacobians.colRange(10, 13).copyTo(Jkk.colRange(28 + image_idx * 6, 31 + image_idx * 6).rowRange(0, 3 * n_points));
			jacobians.colRange(13, 16).copyTo(Jkk.colRange(31 + image_idx * 6, 34 + image_idx * 6).rowRange(0, 3 * n_points));
			...
			jacobians.colRange(4, 10).copyTo(Jkk.colRange(5, 11).rowRange(0, 3 * n_points));
			jacobians.col(16).copyTo(Jkk.col(4).rowRange(0, 3 * n_points));
			...
			dxrdom.copyTo(Jkk.colRange(22, 25).rowRange(3 * n_points, 6 * n_points));
			dxrdT.copyTo(Jkk.colRange(25, 28).rowRange(3 * n_points, 6 * n_points));
			dxrdomckk.copyTo(Jkk.colRange(28 + image_idx * 6, 31 + image_idx * 6).rowRange(3 * n_points, 6 * n_points));
			dxrdTckk.copyTo(Jkk.colRange(31 + image_idx * 6, 34 + image_idx * 6).rowRange(3 * n_points, 6 * n_points));
			jacobians.colRange(0, 2).copyTo(Jkk.colRange(11 + 0, 11 + 2).rowRange(3 * n_points, 6 * n_points));
			jacobians.colRange(2, 4).copyTo(Jkk.colRange(11 + 2, 11 + 4).rowRange(3 * n_points, 6 * n_points));
			jacobians.colRange(4, 10).copyTo(Jkk.colRange(11 + 5, 11 + 11).rowRange(3 * n_points, 6 * n_points));
			jacobians.col(16).copyTo(Jkk.col(11 + 4).rowRange(3 * n_points, 6 * n_points));
