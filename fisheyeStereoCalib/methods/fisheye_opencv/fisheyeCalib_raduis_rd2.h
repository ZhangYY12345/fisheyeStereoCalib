...
namespace my_cv
{
	...
	namespace fisheye_r_rd2
	{
		CV_EXPORTS void projectPoints(cv::InputArray objectPoints, cv::OutputArray imagePoints, const cv::Affine3d& affine,
			cv::InputArray K, cv::InputArray D, double f, double alpha = 0, cv::OutputArray jacobian = cv::noArray(), camMode mode = STEREOGRAPHIC);
		...
		CV_EXPORTS_W void distortPoints(cv::InputArray undistorted, cv::OutputArray distorted, cv::InputArray K, cv::InputArray D, double f, double alpha = 0, camMode mode = STEREOGRAPHIC);

		CV_EXPORTS_W void undistortPoints(cv::InputArray distorted, cv::OutputArray undistorted,
			cv::InputArray K, cv::InputArray D, double f, cv::InputArray R = cv::noArray(), cv::InputArray P = cv::noArray(), camMode mode = STEREOGRAPHIC);
		CV_EXPORTS_W void undistortPoints_H(cv::InputArray distorted, cv::OutputArray undistorted,
			cv::InputArray K, cv::InputArray D, double f, camMode mode = STEREOGRAPHIC);
		...
		CV_EXPORTS_W void estimateNewCameraMatrixForUndistortRectify(cv::InputArray K, cv::InputArray D, double _f, const cv::Size &image_size, cv::InputArray R,
		...
		CV_EXPORTS_W double calibrate(cv::InputArrayOfArrays objectPoints, cv::InputArrayOfArrays imagePoints, const cv::Size& image_size,
			cv::InputOutputArray K, cv::InputOutputArray D, double f, cv::OutputArrayOfArrays rvecs, cv::OutputArrayOfArrays tvecs, int flags = 0,
			cv::TermCriteria criteria = cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 100, DBL_EPSILON));

		CV_EXPORTS_W void stereoRectify(cv::InputArray K1, cv::InputArray D1, double f1, cv::InputArray K2, cv::InputArray D2, double f2, const cv::Size &imageSize, cv::InputArray R, cv::InputArray tvec,
